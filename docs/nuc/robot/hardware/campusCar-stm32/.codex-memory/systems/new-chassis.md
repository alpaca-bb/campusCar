...
- Serial electrical/protocol details:
  ...
  - 5V TTL
  - RX/TX/GND
- Single-board chassis: one STM32 controls all 4 wheels via a single ESC over one UART link.
- Serial port: `/dev/ttyUSB0` (single device, no front/rear split).
...
- `hoverboard_controllers_single.yaml` is set to `open_loop: true` and `position_feedback: false` for the no-encoder chassis.
...
- The probe currently passes all ROS2 dependency checks and finds `hoverboard_driver` at `hoverboard_ws/install/hoverboard_driver`; missing `/dev/ttyUSB0` is expected without an attached serial adapter or when using `--no-devices`.
...
- Confirm serial device naming on the target NUC after the STM32 board is connected.
