...
- STM32 驱动器 USB/TTL 串口、`dialout` 权限、udev 规则
...
|------|---------|------|---------|
| STM32 单 UART 4WD / Hikrobot | `stm32_hoverboard_4wd` | `./scripts/docker_run_stm32.sh` | `docker/compose.stm32_hoverboard.yml` |
...
如果 TTL 串口不是 `/dev/ttyUSB0`，在宿主机项目里写本地覆盖：

```bash
cat > config/profiles/stm32_hoverboard_4wd.local.env <<'EOF'
HOVERBOARD_FRONT_DEVICE="/dev/serial/by-id/<stm32-board>"
EOF
```
