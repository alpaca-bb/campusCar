...
## 2026-08-20 RTK ROS2 Jazzy 接入

- 新增 ROS2 Python 包 `campus_road_inspection_ws/src/cri_drivers/rtk_gps_driver`。
- `rtk_gps_node` 读取 USB 串口 NMEA，发布 `/fix`、`/gps/nmea`，并把完整串口收发字节发布到 `/rtk/serial_rx` 和 `/rtk/serial_tx`；可选 NTRIP RTCM 转发。
- `rtk_serial_monitor` 用于终端直接查看 ASCII/HEX 收发数据，默认不发送设备配置命令。
- 当前 NUC 枚举出 AirM2M 的 `/dev/ttyACM0` 至 `/dev/ttyACM3`，并有 `/dev/serial/by-id/` 稳定链接；实际 GNSS 端口仍需用监视器逐个确认。
- `campusCar/scripts/launch_all.sh` 增加可选 RTK 驱动启动钩子，`RTK_DRIVER_ENABLED` 默认 `0`，避免改变既有车型启动行为。
- 操作文档：`campus_road_inspection_ws/docs/RTK_ROS2接入指南.md`。当前环境没有可用的 `/opt/ros/jazzy`，因此只完成纯 Python/XML/Shell 校验，未执行 colcon 构建或真实串口读取。
