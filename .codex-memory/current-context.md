...
- Current new-chassis focus:
  ...
  - The new direct chassis is STM32 UART based and should use the seller-confirmed `0xABCD` steer/speed/checksum protocol over a single UART; one STM32 board drives all motors.
...
- Current Docker focus:
  ...
  - Software-level new STM32 container probe passes for ROS2 dependencies and `hoverboard_driver`; `/dev/ttyUSB0` still shows missing when running with `--no-devices` or without the chassis connected.
  - Current physical serial state after CH340 binding: the single STM32 board link is mapped locally to `/dev/serial/by-path/pci-0000:00:14.0-usb-0:3.2:1.0-port0` (single-UART chassis, no rear device).
...
- Current branch checkpoint:
  ...
  - Next hardware step after reboot: run `cd ~/campusCar-new-chassis && ./scripts/stm32_hoverboard_probe.sh`, confirm the single STM32 serial device, then run `./scripts/hikrobot_camera_probe.sh` and only then start through the STM32 Docker path or the fixed desktop launcher.
...
- 2026-08-02 checkpoint — NUC side code completeness confirmed, STM32 firmware phase starting:
  ...
  - STM32 firmware MUST implement the seller-confirmed protocol in `.codex-memory/systems/new-chassis.md`: 115200 baud 8N1 5V TTL (RX/TX/GND), command frame `0xABCD + int16 steer + int16 speed + uint16 XOR checksum`, reply frame `SerialFeedbackCompact` = `start + cmd1 + cmd2 + speedR_meas + speedL_meas + batVoltage + boardTemp + cmdLed + checksum`, command range `[-1000, 1000]`, a single STM32 board drives all motors over one UART, current chassis has no encoder ticks (only measured RPM feedback).
  - Open items to confirm with user before firmware coding: STM32 chip model, toolchain (CubeMX/HAL/LL or bare-metal/Keil), motor driver IC, whether encoders exist, UART instance/pins used by the single STM32 board.
